...
const Footer = ({ footer }) => {
  const catchPhrase =
    footer && RichText.render(footer.data.catch_phrase)
      ? RichText.render(footer.data.catch_phrase)
      : '';
  const sns =
    footer && RichText.render(footer.data.sns)
      ? RichText.render(footer.data.sns)
      : '';

  const address =
    footer && RichText.render(footer.data.address)
      ? RichText.render(footer.data.address)
      : '';

  const phone =
    footer && RichText.render(footer.data.phone)
      ? RichText.render(footer.data.phone)
      : '';

  const contact =
    footer && RichText.render(footer.data.contact)
      ? RichText.render(footer.data.contact)
      : '';

  return (
    <footer data-scroll-section>
      <div className="wrapper grid" id="footer-content">
        <div className="underlined-links" id="catch">
          <NextLink href={{ pathname: '/contact' }} passHref>
            <a>{catchPhrase}</a>
          </NextLink>
        </div>
        <div id="addresses" className="underlined-links">
          <NextLink
            href={footer.data.map_link.url}
            target={footer.data.map_link.target}
            id="info-addresses"
          >
            <a>{address}</a>
          </NextLink>
          ...
          <div className="underlined-links" id="social">
            {sns}
          </div>
        </div>
      </div>
          ...
          <div className='logo'>
            <svg xmlns="http://www.w3.org/2000/svg" width="116" height="36.171" viewBox="0 0 116 36.171">
              <g transform="translate(-79 -201.758)">
                <g transform="translate(79 201.758)">
                  <path d="M15.992,113.436l-2.9,7.917-2.878-7.917h-2.1l-2.938,7.876-2.819-7.876H0l3.918,10.675h2.4l2.8-7.357,2.758,7.357h2.4l3.938-10.675Z" transform="translate(0 -108.678)" fill="#09382f" />
                  ...
                  <path d="M65.367,113.217V115.6a3.223,3.223,0,0,0-.579-.06,3.019,3.019,0,0,0-2.279.85,3.388,3.388,0,0,0-.82,2.449v5.177h-2.5V113.337h2.379V114.9a4.164,4.164,0,0,1,3.8-1.679" transform="translate(-26.74 -108.579)" fill="#09382f" />
                  ...
                  <path d="M201.7,114.4a4.656,4.656,0,0,1,1.22,3.5v6.117h-2.5v-5.8a3,3,0,0,0-.66-2.109,2.44,2.44,0,0,0-1.879-.709,2.884,2.884,0,0,0-2.179.829,3.307,3.307,0,0,0-.8,2.389v5.4h-2.5V113.337h2.379v1.379a3.909,3.909,0,0,1,1.559-1.119,5.607,5.607,0,0,1,2.119-.38A4.46,4.46,0,0,1,201.7,114.4" transform="translate(-86.923 -108.579)" fill="#09382f" />
                </g>
                <g transform="translate(79 227.197)">
                  <path d="M71.911,154.23,66.539,164.9H63.753l-5.374-10.67h2.085l4.613,9.148.183-.106,4.566-9.042Z" transform="translate(-58.379 -154.169)" fill="#09382f" />
                  <rect width="1.811" height="10.67" transform="translate(16.014 0.061)" fill="#09382f" />
